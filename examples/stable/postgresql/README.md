...
- Kanister version 0.20.0 installed
...
```console
$ helm install stable/postgresql --name my-release --namespace postgres-test --set image.repository=prasadg193/postgresql --set image.tag=0.6 --set postgresqlPassword=postgres-12345
```
...
$ helm install --name kanister --namespace kasten-io kanister/kanister-operator --set image.tag=0.20.0
```
...
### Create Profile
Create CR if not created already
